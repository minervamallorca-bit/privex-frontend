      ...
      <div style={styles.card}>
        ...
        {mode === 'register' && (
          <div style={styles.inputGroup}>
            ...
            <input style={styles.input} placeholder="Enter Callsign" value={name} onChange={(e) => setName(e.target.value)}/>
          </div>
        )}
        <div style={styles.inputGroup}>
          <label style={styles.label}>{mode === 'register' ? 'CREATE 6-DIGIT PIN' : `WELCOME BACK, ${name.toUpperCase()}`}</label>
          <input style={{...styles.input, textAlign: 'center', letterSpacing: '8px', fontSize: '24px'}} type="password" maxLength="6" placeholder="••••••" value={pin} onChange={(e) => { if (/^\d*$/.test(e.target.value)) setPin(e.target.value); setError(''); }}/>
        </div>
        <button onClick={mode === 'register' ? handleRegister : handleLoginAttempt} style={mode === 'register' ? styles.regBtn : styles.loginBtn}>
            {mode === 'register' ? 'INITIALIZE SYSTEM' : 'UNLOCK TERMINAL'} ➤
        </button>
        {mode === 'login' && (<button onClick={handleReset} style={styles.resetLink}>Reset Device Identity</button>)}
      </div>
    </div>
  );
}
